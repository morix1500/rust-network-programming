...
fn main() {
    ...
    loop {
        ...
        match server_socket.recv_from(&mut recv_buf) {
            ...
            Err(e) => {
                error!("Cloud not recieve a datagram: {}", e);
            }
        }
    }
}
 ...
 * サーバーが返したDHCPOFFERメッセージに対する返答を処理する
 */
fn dhcp_request_message_handler_responded_to_offer(
    ...
    if server_ip != dhcp_server.server_address {
        // クライアントが別のDHCPサーバーを選択した場合 [1]
        info!("Clinet has chosen another dhcp server.");
        return Ok(());
    }
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_ip) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        ...
        match database::select_entry(&con, client_macaddr)? {
            ...
            None => {
                // レコードがないならなにもしてはいけない(RFC2131 P32)
                Ok(())
            }
        }
    } else {
        ...
        // リース延長要求、リース切れによる再要求
        // 本来はこれらの応対で処理を分けるべきだが、簡略化のため同じように処理する
...
fn dhcp_release_message_handler(
    ...
    // 論理削除。DHCPOFFERメッセージを返す際に解放済みのIPアドレスを再割り当てする場合があるから
...
/**
 * 利用可能なIPアドレスを選ぶ
 * 1. 以前そのクライアントにリースされたIPアドレス（開放されたものも含め）
